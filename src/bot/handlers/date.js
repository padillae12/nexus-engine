...
const { getSlotsDisponibles, formatFechaEspanol, formatFechaIngles, formatSlotsParaWhatsApp, toDateStr } = require('../../utils/slots');
...
async function handleDateSelect(sesion, msg) {
  ...
  if (!fecha) {
    return {
      respuesta: isEn
        ? '📅 I couldn\'t understand that date. Try replying with:\n' +
          '• _"tomorrow"_\n• _"on Monday"_\n• _"April 14"_\n• _"15/04"_'
  ...
  if (slots.length === 0) {
    ...
    return {
      respuesta: isEn
        ? `😕 Sorry, there are no time slots available for *${fechaTexto}*.\n\n` +
          `Would you like to try another date?\n\n` +
          `Reply with your preferred date or _"tomorrow"_, _"Monday"_, etc.`
  ...
  if (isEn) {
    return {
      respuesta:
        ...
        `_Reply with the number of your preferred time slot._\n` +
        `_Type *"back"* to choose another service._`,
