...
- **RF-23 (Calendario Interactivo Mensual Plegable y Scroll Continuo):** Componente visual de calendario nativo integrado dinámicamente en el `ListHeaderComponent` de las pantallas de Citas (`index.jsx` y `admin/citas.jsx`). Permite navegación fluida por meses, puntos verdes neón en días con citas, botón para plegar/desplegar la matriz (`[ 🔼 / 🔽 ]`) y desplazabilidad vertical infinita para consultar todas las tarjetas de citas sin obstrucción en pantallas de teléfono de cualquier tamaño.
- **RF-24 (Catálogo Bilingüe y Precios en USD para Turismo Médico):** Campos opcionales de nombre en inglés (`nombre_en`), descripción en inglés (`descripcion_en`) y precios en dólares (`precio_usd`). Al cambiar el bot a inglés, las respuestas despliegan automáticamente los tratamientos en inglés con su costo en USD (y conversión a MXN), configurable libremente desde el panel Admin de la App.
